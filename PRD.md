...
- Цвет фигуры сохраняется на протяжении всей игры (deterministic based on figure ID)
...
**Механизм:**
- Одновременно отображаются 3 фигуры на выбор
- Новые фигуры появляются только после размещения текущей
- При израсходовании одной фигуры генерируется новая фигура
...
**Определение линии:**
- Линия — это непрерывный набор ячеек от одного края поля до противоположного
- В гексагональной сетке существует 6 направлений линий
- Линия считается заполненной, когда все её ячейки заняты

**Направления линий (осевые координаты):**
- Направление 1: (2, -2), (1, -1), (0, 0), (-1, 1), (-2, 2)
- Направление 2: (-2, 2), (-1, 1), (0, 0), (1, -1), (2, -2)
- Направление 3: (2, -1), (1, 0), (0, 1), (-1, 0), (-2, -1)
- Направление 4: (-2, 1), (-1, 0), (0, -1), (1, 0), (2, 1)
- Направление 5: (-2, 1), (-1, 0), (0, -1), (1, 0), (2, -1)
- Направление 6: (-1, -2), (0, -1), (1, 0), (0, 1), (1, 2)

**Уничтожение линий:**
- При размещении фигуры проверяются все 6 направлений
- Все заполненные линии уничтожаются одновременно
- Ячейки освобождаются для дальнейшего использования

### 2.6 Система очков и бонусов

**Базовые очки:**
- За каждую размещённую клетку: 10 очков
- За каждую уничтоженную линию: 100 очков
...
- Итоговый счёт = накопленные очки × бонус заsession
...
├── index.html          (~1 KB)
├── styles.css          (~3 KB)
├── game.js             (~15 KB, основной код)
├── hex-grid.js         (~3 KB, логика сетки)
├── renderer.js         (~4 KB, Canvas rendering)
├── effects.js          (~3 KB, визуальные эффекты)
└── ads.js              (~2 KB, рекламная интеграция)
```
...
- Mobile: 320x480 до 428x926 (portrait/landscape)
- Desktop: 1024x768 до 1920x1080
...
- PlayGama SDK — интеграция с рекламными событиями
